              ...
              <div className="hidden gap-x-4 sm:ms-6 sm:flex sm:items-center">
                ...
                <Dropdown>
                  <DropdownButton as={'button'} className="rounded-full">
                    <Avatar alt="avatar" src={user?.image} className="size-8" width={32} height={32} sizes="32px" />
                  </DropdownButton>
                  <DropdownMenu>
                    {userNavigation.map((item) =>
                      item.name === 'Sign out' ? (
                        <DropdownItem key={item.name} onClick={handleLogout}>
                          {item.name}
                        </DropdownItem>
                      ) : (
            ...
            <div className="border-t border-neutral-200 pt-4 pb-3 dark:border-neutral-700">
              <div className="flex items-center px-4">
                <div className="shrink-0">
                  <Avatar src={user?.image} className="size-10" width={40} height={40} sizes="40px" />
                </div>
                <div className="ms-3">
                ...
                {userNavigation.map((item) =>
                  item.name === 'Sign out' ? (
                    <DisclosureButton
                      ...
                      className="block w-full px-4 py-2 text-start text-base font-medium text-neutral-500 hover:bg-neutral-100 hover:text-neutral-800 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-200"
                    >
                      {item.name}
                    </DisclosureButton>
                  ) : (

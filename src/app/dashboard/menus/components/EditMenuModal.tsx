...
export default function EditMenuModal({
  ...
  // Get available parents (exclude current item and its descendants)
  const getAvailableParents = (): HeaderMenuItem[] => {
    const flattenMenuItems = (items: HeaderMenuItem[], parent: HeaderMenuItem | null = null): HeaderMenuItem[] => {
      ...
      items.forEach(item => {
        // Don't include the current item or its descendants
                ...
                <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  ...
                  <div className="text-sm">
                    {(() => {
                      ...
                      return `${buildPath(selectedParent)} > [${formData.label || 'Updated Item'}]`
                    })()}
                  </div>
                </div>
              )}
                    ...
                    {allMenuItems.map((item) => (
                      <div key={item.id} className="text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-medium ${item.id === editingItem.id ? 'text-blue-600 dark:text-blue-400' : ''}`}>
                            {item.label}
                          </span>
                          <span className="text-xs text-gray-500 dark:text-gray-400">
                            (Level {item.level})
                          ...
                          <div className="ml-4 mt-1">
                            {item.children.map((child) => (
                              <div key={child.id} className="flex items-center gap-2 text-sm">
                                ...
                                <span className={child.id === editingItem.id ? 'text-blue-600 dark:text-blue-400' : ''}>
                                  {child.label}
                                </span>
                                <span className="text-xs text-gray-500 dark:text-gray-400">
                                  (Level {child.level})
